...
export class AuthService {
  ...
  private getRoles(): string[] {
    const rolesString = localStorage.getItem(this.ROLES);
    if (rolesString) {
      return rolesString.split(',');
    }
    return [];
  }
  getChefFilProfile(): Observable<any> {
    const username = this.loggedUser;
  ...
  getEtudiantProfile(): Observable<any> {
    const username = this.loggedUser;
    return this.http.get<Etudiant>(`${environment.apiUrl}/etudiants/username?username=${username}`);
  }

  getAdminProfile(): Observable<any> {
    const username = this.loggedUser;
    return this.http.get<Admin>(`${environment.apiUrl}/admins/login?login=${username}`);
  }
  getEncadrantPedagogiqueProfile(): Observable<any> {
    const username = this.loggedUser;
